...
eqpolicy(t)..                        cs(t)**(-gammae)  =E=  sum((nka,nza)$smol(nka,nza), coeffe(nka,nza)*phik(nka,t) * phiz(nza,t)) + eulerer(t) ;

eqcne(t,nz)..                        cne(t,nz)**(-gammae) =E=  ( sum((nka,nza)$smol(nka,nza), coeffe(nka,nza)*phik(nka,t+1) * phizne(nza,t,nz))
                                                               )$(ord(t) LT card(t))
...
gq("1")=-2.8570; gq("2")=-1.3556; gq("3")=0;  gq("4")=1.3556; gq("5")= 2.8570;
prob("1")=0.0113; prob("2")=0.2221; prob("3")=0.5333; prob("4")=0.2221; prob("5")=0.0113;

err0(nz) = gq(nz);
z0(nz) = gq(nz)*sigmazsv("2") ;
*z0(nz) = gq(nz)*0.04 ;
...
Parameters
coeffs(nka,nza) ;
coeffs(nka,nza) = coeff.l(nka,nza)$(abs(coeff.l(nka,nza)) GE 1.0E-3) ;
display coeffs
;
$offtext

coeffe.l("1","1") = (sum(t, cs(t))/card(t) )**(-gammae.l) ;
*coeffe.l(nka,nza) = coeffs(nka,nza)$smol(nka,nza);
*coeffe.up(nka,nza) = 50;
*coeffe.lo(nka,nza) = -50;
...
erreulernesv("1")       = -0.001;
erreulernesv("3")       = 0.001;
*smol(nka,nza) = 1 ;
*solve estimation using nlp maximising entropie;
